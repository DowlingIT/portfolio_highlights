...
We never found the root cause. Months of investigation pointed at a worn-out OS we could nudge but not name. So the work changed shape. Instead of fixing the server, we taught the application not to need the server's memory.
...
A useful side benefit: with sessions out of any one server's memory, the application can also run in a true web farm. The client stayed.
